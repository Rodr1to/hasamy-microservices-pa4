...
@RestController
@RequestMapping("/api/catalogo/estadocivil")
...
public class EstadoCivilController {
    ...
    public ResponseEntity<EstadoCivilEntity> actualizar(@PathVariable Integer id, @RequestBody EstadoCivilEntity entity) {
        return service.buscarPorId(id).map(ec -> {
            ec.setNombre(entity.getNombre());
            ec.setEstado(entity.getEstado());
            return ResponseEntity.ok(service.actualizar(ec));
        }).orElse(ResponseEntity.notFound().build());
    }
